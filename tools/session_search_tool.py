...
SESSION_SEARCH_SCHEMA = {
    "name": "session_search",
    "description": (
        "Search past Hermes sessions (local SQLite FTS5) or scroll inside one. "
        "No LLM calls; returns actual stored messages.\n\n"
        "SOURCE-FIRST LIMIT: this searches Hermes conversation history only — not "
        "evidence about what an external source currently contains. If the user gave "
        "a direct source (URL, contact, file, account, live system), inspect it "
        "first when accessible; use session_search as secondary context. Never "
        "conclude 'not found' or 'no prior correspondence' from session history "
        "alone when a direct source was provided.\n\n"
        "SHAPES (inferred from which args are set):\n"
        "1) DISCOVERY — query=\"auth refactor\" (+limit, sort): FTS5 top-N sessions; "
        "each has session_id, snippet, match_message_id, messages = ±5 around the "
        "hit, and bookend_start/bookend_end (first/last 3 messages of the session).\n"
        "2) SCROLL — session_id + around_message_id (+window): ±window messages "
        "around the anchor. To scroll FORWARD pass messages[-1].id as the next "
        "anchor; backward, messages[0].id. messages_before/after < window = session "
        "edge.\n"
        "3) READ — session_id only: whole session (first 20 + last 10 when large). "
        "Resolves @session:<profile>/<id> links: profile=<profile>, session_id=<id>.\n"
        "4) BROWSE — no args: recent sessions (titles, previews, timestamps).\n\n"
        "FTS5: multi-word = AND. Also OR, \"quoted phrases\", NOT, prefix* wildcards."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "query": {
                "type": "string",
                "description": (
                    "DISCOVERY: FTS5 query (keywords/boolean). Omit to browse; "
                    "ignored in scroll shape."
                ),
            },
            "limit": {
                "type": "integer",
                "description": "DISCOVERY: max sessions returned (default 3, max 10).",
                "default": 3,
            },
            "sort": {
                "type": "string",
                "enum": ["newest", "oldest"],
                "description": (
                    "DISCOVERY: temporal bias. 'newest' = where-did-we-leave-X, "
                    "'oldest' = how-did-X-start; omit for pure relevance."
                ),
            },
            "session_id": {
                "type": "string",
                "description": (
                    "Target session for scroll/read (from a discovery result or "
                    "@session link)."
                ),
            },
            "around_message_id": {
                "type": "integer",
                "description": (
                    "SCROLL: message id to center on (match_message_id, or a prior "
                    "window's edge id to page)."
                ),
            },
            "window": {
                "type": "integer",
                "description": "SCROLL: messages each side of the anchor, clamped [1,20], default 5.",
                "default": 5,
            },
            "role_filter": {
                "type": "string",
                "description": (
                    "Comma-separated roles. Default 'user,assistant'; add 'tool' to "
                    "search tool output."
                ),
            },
            "profile": {
                "type": "string",
                "description": (
                    "Read another profile's DB (read-only), e.g. the profile segment "
                    "of an @session link. Omit for current."
                ),
            },
        },
        "required": [],
