...
def current_context_usage_tokens(agent: Any, messages: list, system_prompt: str = "") -> int:
    """Return the same "usage" number the in-turn threshold check uses.

    The synchronous compaction paths (conversation_loop.py post-tool-loop,
    turn_context.py preflight) decide on:

      * the provider-reported ``last_prompt_tokens`` when it is > 0 (the
        authoritative context size the provider actually billed), else
      * a rough ``estimate_request_tokens_rough(messages, tools=...)`` fallback
        (covers the "provider returned no usage / disconnect" case, #2153).

    A sentinel of -1 means "compaction just ran and no real provider prompt
    count has arrived yet" — the in-turn path treats that as 0 (avoid acting on
    a schema-heavy post-compression rough estimate, #36718), and so do we.
    Reusing this keeps the background soft-threshold decision on the SAME
    accounting as the hard backstop — we never introduce a second tokenizer
    path that could disagree with it.
    """
    compressor = getattr(agent, "context_compressor", None)
    if compressor is None:
        return 0
    last = getattr(compressor, "last_prompt_tokens", 0)
    if last > 0:
        return int(last)
    if last == -1:
        # Compaction just happened; real usage not yet reported. Not pressure.
        return 0
    try:
        return int(
...
def should_background_compress(
    *,
    usage_tokens: int,
    threshold_tokens: int,
    soft_ratio: float,
) -> bool:
    """Pure soft-threshold arming decision for post-delivery compression.

    Returns True only in the soft band: ``soft_ratio * threshold`` (inclusive)
    up to ``threshold`` (EXCLUSIVE). At/above the hard ``threshold`` the
    synchronous in-turn path already fired (or will on the next real call), so
    the background pass must stand down and leave the hard backstop untouched —
    returning False there is what guarantees "exactly one compression" for a
    single logical over-threshold state (the in-turn one). Below the soft band
    there is nothing to pre-compact.

    Split out as a pure function (no agent, no I/O) so the arming math is unit
    testable in isolation — mirrors the ``_should_run_preflight_estimate`` seam.
    """
    if threshold_tokens <= 0:
        return False
    if not (0.0 < soft_ratio < 1.0):
        return False
    soft_threshold = int(threshold_tokens * soft_ratio)
    return soft_threshold <= usage_tokens < threshold_tokens


def maybe_background_compress(
    agent: Any,
    messages: list,
    system_message: str,
    *,
    ...
    """Run a soft-threshold background compaction if armed. Returns whether it ran.

    Called from the gateway AFTER a turn's final response has been delivered
    and only when the session is idle (no active run) — see
    gateway/run.py::_schedule_background_compression_after_turn. Blocking; the
    caller runs it off the event loop in the gateway executor.

    Contract with the hard backstop (this is the whole point of the feature):

      * We only enter the soft band (below the hard threshold), so we never
        race the in-turn path for the SAME over-threshold state.
      * ``compress_context`` takes the state.db-backed per-session compression
        lock. If a real turn started between our arming check and here and is
        mid-compress, our acquire fails and ``compress_context`` returns the
        messages unchanged — a safe no-op. Exactly one compression wins.

    Never raises into the caller: any failure is swallowed so a broken idle
    pass can never affect a user turn.
    """
    if not getattr(agent, "compression_enabled", False):
        return False
    if not getattr(agent, "compression_background", False):
        return False
    compressor = getattr(agent, "context_compressor", None)
    if compressor is None:
        return False
    threshold = int(getattr(compressor, "threshold_tokens", 0) or 0)
    soft_ratio = float(getattr(agent, "compression_soft_ratio", 0.8) or 0.8)
    usage = current_context_usage_tokens(agent, messages, system_message)
    if not should_background_compress(
        usage_tokens=usage, threshold_tokens=threshold, soft_ratio=soft_ratio
    ):
        return False
    # Respect the compressor's own anti-thrash / cooldown guards: if it would
    # decline a same-usage compress in-turn, don't background-compress either.
    try:
        if not compressor.should_compress(usage):
            return False
    except Exception:
        return False
    logger.info(
        "background compression armed: session=%s usage=~%s soft=%s hard=%s",
        getattr(agent, "session_id", None) or "none",
        f"{usage:,}",
        f"{int(threshold * soft_ratio):,}",
        f"{threshold:,}",
    )
    try:
        new_messages, _new_sp = compress_context(
            agent,
            messages,
            system_message,
            approx_tokens=usage,
        ...
        logger.debug("background compression failed (non-fatal): %s", exc)
        return False
    # compress_context returns the input unchanged on a no-op (lost the lock,
    # summary aborted). Only treat a real size change as "compressed".
    return len(new_messages) != len(messages)


def prewarm_after_compression(
    agent: Any,
    messages: list,
    *,
    timeout: float = 120.0,
) -> bool:
    """Fire one max_tokens=1 warm-up so the server re-prefills the new prefix.

    CRITICAL byte-identical-prefix rule: a warm-up only populates the server's
    prefix cache for the next real call if its templated prompt is byte-for-byte
    identical to that call's prefix. We therefore build kwargs through the SAME
    path the real request uses (``agent._build_api_kwargs`` →
    ``build_api_kwargs``), with the post-compaction message list and the SAME
    tools / system prompt / sampling params, and override ONLY ``max_tokens``
    (to 1) and ``stream`` (to False). Tools matter: they are templated into the
    prompt server-side, so dropping them would warm a DIFFERENT prefix than the
    real call and defeat the cache.

    Fire-and-forget with a hard timeout. Any failure (build, transport,
    timeout) is debug-logged and ignored — this must never block or delay a
    user turn, and never raise into the caller. Returns True only when the
    warm-up request completed without error.

    Restricted to the ``chat_completions`` api_mode: that is the local-vLLM
    deployment this targets, and it is the mode whose ``build_api_kwargs``
    output is stable across two calls (the Anthropic/Codex/Bedrock transports
    consume ephemeral state and mint per-request ids, so a second build is not
    guaranteed byte-identical). Other modes simply skip the warm-up.
    """
    if not getattr(agent, "compression_prewarm", False):
        return False
    if getattr(agent, "api_mode", "chat_completions") != "chat_completions":
        return False
    try:
        api_kwargs = agent._build_api_kwargs(list(messages))
    except Exception as exc:
        logger.debug("prewarm: build_api_kwargs failed (non-fatal): %s", exc)
        return False
    if not isinstance(api_kwargs, dict):
        return False
    # Override ONLY max_tokens and stream — everything else (messages, tools,
    # temperature, extra_body, session metadata) stays exactly as the real
    # next call would send it. Use the provider-correct max-tokens param name
    # so the value actually lands (some routes use max_completion_tokens): pop
    # any max-tokens key build produced, then re-set to 1 via the same helper.
    try:
        _mt_param = agent._max_tokens_param(1)
    except Exception:
        _mt_param = {"max_tokens": 1}
    for _k in ("max_tokens", "max_completion_tokens"):
        api_kwargs.pop(_k, None)
    api_kwargs.update(_mt_param)
    api_kwargs["stream"] = False
    try:
        client = agent._ensure_primary_openai_client(reason="kv_prewarm")
    except Exception as exc:
        logger.debug("prewarm: client unavailable (non-fatal): %s", exc)
        return False
    ...
    def _do_call() -> None:
        try:
            ...
            _result["ok"] = True
        except Exception as _exc:  # noqa: BLE001 — swallow every warm-up error
            _result["error"] = _exc

    _t = threading.Thread(target=_do_call, name="kv-prewarm", daemon=True)
    _t.start()
    _t.join(timeout=max(0.1, float(timeout)))
    if _t.is_alive():
        # Timed out — abandon it. The daemon thread will exit when the
        # request settles; we never block a user turn on it.
        logger.debug(
            "prewarm: warm-up exceeded %.0fs timeout — abandoned (non-fatal)",
            float(timeout),
        )
        return False
    if _result.get("ok"):
        logger.debug(
            "prewarm: warmed new prefix for session=%s (max_tokens=1)",
            getattr(agent, "session_id", None) or "none",
        )
        return True
    logger.debug(
        "prewarm: warm-up failed (non-fatal): %s", _result.get("error")
    )
    return False
